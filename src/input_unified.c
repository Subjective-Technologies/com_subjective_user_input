...
static int start_local_server(int port, pid_t *server_pid) {
    ...
    /* Normalize project root if running from build output */
    strncpy(project_root, cwd, sizeof(project_root) - 1);
    project_root[sizeof(project_root) - 1] = '\0';
    if (path_ends_with(project_root, "/build/linux")) {
        project_root[strlen(project_root) - strlen("/build/linux")] = '\0';
    } else if (path_ends_with(project_root, "/build/windows")) {
        project_root[strlen(project_root) - strlen("/build/windows")] = '\0';
    } else if (path_ends_with(project_root, "/build/macos")) {
        project_root[strlen(project_root) - strlen("/build/macos")] = '\0';
    } else if (path_ends_with(project_root, "/build")) {
        project_root[strlen(project_root) - strlen("/build")] = '\0';
    }
    
    /* Find server script */
    int ret = snprintf(script_path, sizeof(script_path), "%s/server/input_server.py", project_root);
    ...
    ret = snprintf(python_path, sizeof(python_path), "%s/venv/bin/python3", project_root);
    ...
    char logs_dir[1024];
    ret = snprintf(logs_dir, sizeof(logs_dir), "%s/logs", project_root);
    ...
    if (pid == 0) {
        ...
        ret = snprintf(server_dir, sizeof(server_dir), "%s/server", project_root);
        if (ret < 0 || (size_t)ret >= sizeof(server_dir)) {
            exit(1);
        }
        setenv("PYTHONPATH", server_dir, 1);
        setenv("LOGS_FOLDER_PATH", logs_dir, 1);
...
static int start_local_server(int port, HANDLE *server_process, DWORD *server_pid) {
    ...
    /* Find Python executable */
    ret = snprintf(python_path, sizeof(python_path), "%s\\venv\\Scripts\\python.exe", project_root);
    if (ret < 0 || (size_t)ret >= sizeof(python_path)) {
        LOG_ERROR("Path too long for python_path buffer");
        return -1;
    }
    ...
    /* Ensure server modules are importable */
    snprintf(server_dir, sizeof(server_dir), "%s\\server", project_root);
    SetEnvironmentVariableA("PYTHONPATH", server_dir);
    snprintf(logs_dir, sizeof(logs_dir), "%s\\logs", project_root);
    SetEnvironmentVariableA("LOGS_FOLDER_PATH", logs_dir);
