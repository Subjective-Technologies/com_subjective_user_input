...
|---------|-------|---------|-------|
| Input Capture | ✅ evdev | 🔧 Raw Input (stub) | 🔧 IOKit (stub) |
| Input Injection | ✅ XTest | 🔧 SendInput (stub) | 🔧 CGEvent (stub) |
...
The Windows stubs in the code need to be expanded:

1. **Input Capture**: Implement `RegisterRawInputDevices()` and message loop
2. **Input Injection**: Complete `SendInput()` with proper key code mapping
3. **Monitor Detection**: Implement `EnumDisplayMonitors()` callback
